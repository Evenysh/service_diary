...
if entry_date > today:
    status = "Нельзя сохранить запись: указана будущая дата"
elif mood_score < 1 or mood_score > 10:
    status = "Нельзя сохранить запись: оценка настроения должна быть от 1 до 10"
elif category != "личное" and category != "учёба" and category != "работа":
    status = "Нельзя сохранить запись: неизвестная категория"
else:
    status = "Запись можно сохранить в дневник"
